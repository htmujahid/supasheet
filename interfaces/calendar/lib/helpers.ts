...
import type { CalendarCell, Event, CalendarView, VisibleHours, WorkingHours } from "@/interfaces/calendar/lib/types";
...
export function getVisibleHours(visibleHours: VisibleHours, singleDayEvents: Event[]) {
...
export function calculateMonthEventPositions(multiDayEvents: Event[], singleDayEvents: Event[], selectedDate: Date) {
  ...
  sortedEvents.forEach(event => {
    ...
    if (position !== -1) {
      ...
      eventPositions[Object.values(event.pk).join("/")] = position;
    }
  });
...
export function getMonthCellEvents(date: Date, events: Event[], eventPositions: Record<string, number>) {
  ...
  return eventsForDate
    .map(event => ({
      ...event,
      position: eventPositions[Object.values(event.pk).join("/")] ?? -1,
