...
import { EventBullet } from "@/interfaces/calendar/components/month-view/event-bullet";
import { MonthEventBadge } from "@/interfaces/calendar/components/month-view/month-event-badge";

import { cn } from "@/lib/utils";
import { getMonthCellEvents } from "@/interfaces/calendar/lib/helpers";

import type { CalendarCell, Event } from "@/interfaces/calendar/lib/types";
    ...
    <div className={cn("flex h-full flex-col gap-1 border-l border-t py-1.5 lg:pb-2 lg:pt-1", isSunday && "border-l-0")}>
      ...
      <div className={cn("flex h-6 gap-1 px-2 lg:h-[94px] lg:flex-col lg:gap-2 lg:px-0", !currentMonth && "opacity-50")}>
        {[0, 1, 2].map(position => {
          ...
          const eventKey = event ? `event-${Object.values(event.pk).join("/")}-${position}` : `empty-${position}`;
