...
import { dataTableConfig } from "@/interfaces/data-table/config/data-table.config";
import {
  getDefaultFilterOperator,
  getFilterOperators,
} from "@/interfaces/data-table/lib/data-table";
import { generateId } from "@/interfaces/data-table/lib/id";
import { getFiltersStateParser } from "@/interfaces/data-table/lib/parsers";
import type {
...
type ResourceFilterListProps = React.ComponentProps<typeof PopoverContent> & {
...
export function ResourceFilterList<TData>({
  columns,
  ...
  const [filters, setFilters] = useQueryState(
    FILTERS_KEY,
    getFiltersStateParser<TData>(columns.map((field) => field.label))
      .withDefault([])
      .withOptions({
        clearOnDefault: true,
        shallow,
          ...
          <PopoverTrigger asChild>
            ...
            >
              <span className="truncate">
                {columns.find((column) => column.label === filter.id)?.label ??
                  "Select field"}
              </span>
  ...
  if (filter.operator === "empty" || filter.operator === "not.empty") {
    return (
      <div
        id={inputId}
        role="status"
        aria-label={`${column?.label} filter is ${
      ...
      return (
        ...
        >
          <SelectTrigger
            ...
            aria-label={`${column?.label} boolean filter`}
        ...
        >
          <FacetedTrigger asChild>
            <Button
              id={inputId}
              aria-controls={inputListboxId}
              aria-label={`${column?.label} filter value${multiple ? "s" : ""}`}
              variant="outline"
              size="sm"
        ...
        <Popover open={showValueSelector} onOpenChange={setShowValueSelector}>
          <PopoverTrigger asChild>
            <Button
              id={inputId}
              aria-controls={inputListboxId}
              aria-label={`${column?.label} date filter`}
              variant="outline"
              size="sm"
              className={cn(
          ...
          >
            {filter.operator === "between" ? (
              <Calendar
                aria-label={`Select ${column?.label} date range`}
                mode="range"
                initialFocus
                selected={
                  dateValue.length === 2
