...
// URL state keys (following use-data-table.ts patterns)
const PAGE_KEY = "page";
...
interface DataGridState {
  // Note: sorting and columnFilters are now managed via nuqs URL state
...
interface UseDataGridProps<TData>
  ...
  pageCount?: number;
  // nuqs options (following use-data-table.ts patterns)
  ...
  onCellUpdate?: (update: Array<UpdateCell>) => void;
}
...
function useDataGrid<TData>({
  ...
  shallow = true,
  startTransition,
  enableAdvancedFilter: _enableAdvancedFilter = false,
  onCellUpdate,
  ...props
  ...
  // nuqs query state options (following use-data-table.ts patterns)
  ...
  // Build column IDs set for parser validation
  ...
  const [perPage, setPerPage] = useQueryState(
    ...
    parseAsInteger
      ...
      .withDefault(initialState?.pagination?.pageSize ?? 100),
  );
  ...
  // Filters state via nuqs (advanced filter mode)
  ...
  // columnFilters derived from filters for TanStack Table compatibility
  ...
  // Note: sorting and columnFilters now come from nuqs URL state (defined above)
      ...
      try {
        ...
        for (
          ...
          for (
            ...
            const cellOpts = column?.columnDef?.meta;
        ...
        if (updates.length > 0) {
          ...
          if (currentState.cutCells.size > 0) {
            for (const cellKey of currentState.cutCells) {
              ...
              const cellVariant = column?.columnDef?.meta?.variant;
  ...
  const onCellMouseEnter = useCallback(
    (rowIndex: number, columnId: string, _event: MouseEvent) => {
      const currentState = store.getState();
      if (
  ...
  const getMemoizedSortedRowModel = useMemo(
    () => getSortedRowModel(),
    [],
  );
  ...
  const tableState = useMemo<Partial<TableState>>(
    ...
    [propsRef, pagination, sorting, columnFilters, rowSelection],
  );

  const getMemoizedPaginationRowModel = useMemo(
    () => getPaginationRowModel(),
    [],
  );
  ...
  return useMemo(
    () => ({
      ...
      // nuqs state for downstream components
      pagination,
      shallow,
      debounceMs,
      throttleMs,
      // Filters for DataTableFilterList
      filters,
