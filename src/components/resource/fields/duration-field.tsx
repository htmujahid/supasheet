...
export function DurationField({ columnMetadata }: FieldProps) {
  ...
  useEffect(() => {
    const ms = parseInt(field.state.value as string) || 0
    const totalSeconds = ms / 1000

    const h = Math.floor(totalSeconds / 3600)
    const m = Math.floor((totalSeconds % 3600) / 60)
    const s = parseFloat((totalSeconds % 60).toFixed(3))
